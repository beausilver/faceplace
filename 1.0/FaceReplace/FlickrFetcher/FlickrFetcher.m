...
+ (int)getGlobalNetCount{
    
    return count;
}

+ (void) modGlobalNetCount:(int)byInt{
    count += byInt;
}
...
+ (NSDictionary *)executeFlickrFetch:(NSString *)query
{
    ...
    if ([self getGlobalNetCount] == 0){
        [[UIApplication sharedApplication] setNetworkActivityIndicatorVisible:YES];
    }
    [self modGlobalNetCount:1];
    
    NSData *jsonData = [[NSString stringWithContentsOfURL:[NSURL URLWithString:query] encoding:NSUTF8StringEncoding error:nil] dataUsingEncoding:NSUTF8StringEncoding];
    NSError *error = nil;
    NSDictionary *results = jsonData ? [NSJSONSerialization JSONObjectWithData:jsonData options:NSJSONReadingMutableContainers|NSJSONReadingMutableLeaves error:&error] : nil;
    if ([self getGlobalNetCount] == 1){
...
//facereplace id is: 93790769@N04
//beausilver id is: 80693547@N08

+ (NSArray *)faceReplacePhotos
{
    NSString *request = @"http://api.flickr.com/services/rest/?user_id=93790769@N04&format=json&nojsoncallback=1&extras=original_format,tags,description,geo,date_upload,owner_name&page=1&method=flickr.photos.search";
